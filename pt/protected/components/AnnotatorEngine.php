...
class AnnotatorEngine {
	 ...
	 *
	 * @param string $char        	
	 * @return Char the corresponding char for the given string
	 */
	...
	public static function loadPhrasesFromDictionaries($char, $compounds, $dictionariesID) {
		...
		// note this method has not been really tested with a single schema - if the single schema is ever recovered, it has to be tested
		...
		foreach ( $compounds as $searchFull ) {
			if ($mode > 1) {
				if (is_null($begin))
					$begin=$char . mb_substr($searchFull, 0, 1, $encoding);
				;
				
				$search=mb_substr($searchFull, $mode - 1, NULL, $encoding);
			} else {
				$search=$searchFull;
			}
		...
		$criteriaSimplified->addInCondition('dictionaryId', $dictionariesID); // it has to be in one of the chosen dictionaries
		$criteriaTraditional->addInCondition('dictionaryId', $dictionariesID); // it has to be in one of the chosen dictionaries
		                                                                       
		// the first letter (because of the DB structure)
		$criteriaSimplified->compare('simplified_begin', $begin);
	...
	public function loadPhrasesDouble($char, $offset) {
		if (DictEntryPhrase::getFirstPartLength() != 2)
	...
	public static function createFormatter($dictID) {
		$result=array();
		$dict=Dictionary::model()->findByAttributes(array(
				'id' => $dictID
		));
	 ...
	 *
	 * @param int $lineIndex
	 *        	which line is being outputed
	 * @param int $templateIteration
	 *        	one-based index of the current template
	 * @param boolean $finish
	 *        	if all following lines should be outputed as well
	 */
